...
## Windows package result — acceptance blocked
...
- Next: isolate import/type-data caches and URP assets in the disposable copy; if needed compare with an independently imported copy or an explicitly agreed engine version. Do not upgrade the source project or merge this branch on the strength of Editor tests alone. Then rerun `--validate-map`, rendered route acceptance, and the remaining map-run integration.
